...
void TDMA_TDD_RX::setvalue(bool value){
...
void TDMA_TDD_RX::run(){
    ...
    bool is_slot_allocation_finished=false;
    int index_slot=0;
    bvec slot_vector(tdma_slots);
    slot_vector.zeros();
    ...
    while(!stop_signal){
        ...
        if((is_time_set==false)&&(first_tx_timestamp==true)){
            ...
            //if(is_slot_allocation_finished){
            ...
            /*}
                ...
                if((packet_ok)||(first_tx_timestamp==false)){
                    is_slot_allocation_finished=true;
                    cout << "packet_ok " << packet_ok <<  " first_tx_timestamp " << first_tx_timestamp << endl;
                    emit slot_allocation(0);
                }
                else{

                    if(preamble_ok)
                        slot_vector[index_slot]=bin(1);
                    cout << index_slot << " " << slot_vector[index_slot] << endl;
                    index_slot=index_slot+1;
                    timestamp=timestamp+time_gap+index_slot*time_gap/tdma_slots;
                    rx_buff=device->readsamples(Number_of_received_symbols,timestamp);
                    if(index_slot>=tdma_slots){
                            int i=0;
                            bool slot_not_found=true;
                            while((i<tdma_slots/2)&&(slot_not_found)){
                                if((slot_vector[i]==0)&&(slot_vector[i+tdma_slots/2]==0)){
                                    slot_not_found=false;
                                    timestamp=timestamp+time_gap+i*time_gap/tdma_slots;
                                    emit slot_allocation(i);
                                }
                                i++;
                            }
                            if(slot_not_found)
                                emit slot_allocation(tdma_slots);
                    }
                }
            }*/
        }
        if((rx_buff!=previous_rx_buff)&&(rx_buff[0]!=0.0)){
            ...
            if(preamble_ok==true){
               ...
               if((is_time_set==false)&&(first_tx_timestamp==true)){
                        ...
                        cout << "TIME HAS BEEN SET FOR SYNCHRONIZATION " << timestamp << endl;
