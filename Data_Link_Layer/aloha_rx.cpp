    ...
    while(!stop_signal){
        ...
        if((rx_buff!=previous_rx_buff)&&(rx_buff[0]!=0.0)){
            ...
            if(waveform==1){
                ...
                //cout << "preamble_ok " << preamble_ok << " " << preamble_start << endl;
                //cout << "ack_ok " << ack_ok << " " << ack_start << endl;
            ...
            if(preamble_ok==true){
                if((packet_ok)&&(((ack_start<preamble_start)&&(ack_address_ok))||((ack_start<preamble_start)&&(!ack_address_ok)))){
                    //cout << "ack_start " << ack_start << endl;
                    //cout << "preamble_start " << preamble_start << endl;
                    //cout << "ack_address_ok " << ack_address_ok << endl;
                    //Tell transmitter that we have received a packet correctly to send an ack
                    emit sendack(packet->packetnorx);
                    //cout << "emit ACK " << packet->packetnorx << endl;
