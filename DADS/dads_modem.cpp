...
cvec DADS_Modem::modulate(bvec data_packet)
{
    BPSK bpsk;
    ...
    vec sum_chips;
    sum_chips.set_size(transmitted_chips.size()+delay);
    sum_chips=1./sqrt(2)*concat(spread.spread(reference),zeros(delay))+1./sqrt(2)*concat(zeros(delay),transmitted_chips);
...
bvec DADS_Modem::demodulate(cvec rx_buff, int OF, int &time_offset)
{
    BPSK bpsk;
    ...
    for(int l=0;l<rx_buff.size()/SF_OF/qf;l++){
        temp_received_chips.zeros();
        for(int k=l*qf;k<(l+1)*qf;k++){
            for(int m=0;m<SF/SF_pseudo;m++){
                temp_received_chips=temp_received_chips+rx_buff.get(k*SF_OF+m*SF_pseudo_OF,k*SF_OF+(m+1)*SF_pseudo_OF-1)/qf/(SF/SF_pseudo);
            }
        }
        for(int k=0;k<qf;k++){
            for(int m=0;m<SF/SF_pseudo;m++){
                noise_received_chips.replace_mid(l*SF_OF*qf+k*SF_OF+m*SF_pseudo_OF,temp_received_chips);
            }
        }
    }
    ...
    for(int k=0;k<shifted_received_chips.size()/SF_OF;k++){
           metric(k,0) = sum(real(shifted_received_chips.get(k*SF_OF+delay_OF,(k+1)*SF_OF+delay_OF-1)));
           metric2(0)=metric2(0)+sign(metric(k,0))*metric(k,0);
    }

    for(int j=1;j<SF_OF;j++){
        for(int k=0;k<shifted_received_chips.size()/SF_OF-1;k++){
               metric(k,j) = metric(k,j-1)+real(shifted_received_chips.get((k+1)*SF_OF+delay_OF+j-1))-real(shifted_received_chips.get(k*SF_OF+delay_OF+j-1));
...
bool DADS_Modem::preamble_detection(bvec received_bits,bvec &received_bits2,int &preamble_start){
...
bvec DADS_Modem::charvec2bvec(vector<char> input){
