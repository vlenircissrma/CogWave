...
DADS_Framing::DADS_Framing(int nb_bits){
    ...
    fec = new DADS_FEC();
    file_video = new DADS_Multimediaread((char*)"video_inputpipe",false);
    file_audio = new DADS_Multimediaread((char*)"audio_inputpipe",false);
    file_text = new DADS_Multimediaread((char*)"text_inputpipe",true);
...
void DADS_Framing::restart_video(){
...
void DADS_Framing::restart_audio(){
...
bvec DADS_Framing::encode_frame(int my_adress,int dest_adress, int &nb_read)
{

    bvec data_packet(frame_size);
    data_packet=randb(frame_size);
    nb_read=0;

    //cout << frame_size << endl;
    vector<char> information(8);
    information[0]='!';
    information[1]='!';
    information[2]='H';
   ...
   //cout << "Number of characters read " << nb_read << endl;
    ...
    if((information_bits.size()+delimiter_bits.size()+merge_transmitted_bits.size()<data_packet.size())&&(merge_transmitted_bits.size()!=0)){
        data_packet.replace_mid(8,information_bits);
        data_packet.replace_mid(8+8*8,merge_transmitted_bits+scrambling);
        data_packet.replace_mid(8+8*8+merge_transmitted_bits.size(),delimiter_bits);
...
bool DADS_Framing::decode_frame(bvec received_bits, int my_adress, int &src_adress)
{
    ...
    int i=0;
    while((i<int(information.size())-8)&&(packet_ok==false)){
        if((information[i]=='!')&&(information[i+1]=='!')&&(information[i+2]=='H')&&(information[i+5]=='!')&&(information[i+6]=='!')&&(information[i+7]=='H')){
            ...
            if(('0'+ my_adress)==information[i+4]){
                ...
                while((j<int(information.size())-3)&&(end_delimiter==false)){
                    if((information[j]=='!')&&(information[j+1]=='!')&&(information[j+2]=='D')){
                        ...
                        vector<char> buff(j-(i+8));
                        for(int k=0;k<j-(i+8);k++)
                            buff[k]=information[i+8+k];


                        bvec merge_received_bits=charvec2bvec(buff);
                        RNG_reset(0);
                        bvec scrambling=randb(merge_received_bits.size());
...
bvec DADS_Framing::charvec2bvec(vector<char> input){
...
vector<char> DADS_Framing::bvec2charvec(bvec input){
