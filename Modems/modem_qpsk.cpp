...
cvec Modem_QPSK::modulate(bvec data_packet)
{
    ...
    gr_pfb_arb_resampler_ccf_sptr d_arb_pfb;
    int nfilts=32;
    float roll_off=0.5;
    ...
    d_arb_pfb = gr_make_pfb_arb_resampler_ccf(OF,taps);
...
cvec Modem_QPSK::costas_mm(cvec received_samples){
    ...
    digital_mpsk_receiver_cc_sptr d_clock_qpsk_mm;
    ...
    d_clock_qpsk_mm = digital_make_mpsk_receiver_cc(M,theta,loop_bw,fmin,fmax,mu,gain_mu,omega,gain_omega,omega_rel);
...
cvec Modem_QPSK::costas_pfb(cvec received_samples){
    ...
    digital_pfb_clock_sync_ccf_sptr d_clock_qpsk_pfb;
    digital_fll_band_edge_cc_sptr d_qpsk_fll;
    digital_constellation_receiver_cb_sptr d_constellation_qpsk_pfb;
    ...
    d_qpsk_fll = digital_make_fll_band_edge_cc(OF,roll_off,55,loop_bw);
    d_clock_qpsk_pfb = digital_make_pfb_clock_sync_ccf(OF,loop_bw,taps2);
    digital_constellation_qpsk_sptr d_qpsk;
    d_qpsk = digital_make_constellation_qpsk();
    d_constellation_qpsk_pfb = digital_make_constellation_receiver_cb(d_qpsk,loop_bw,fmin,fmax);


    //Frequency Locked Loop
    int noutput_items=received_samples.size();
    d_qpsk_fll->d_fllbuffer.reserve(d_qpsk_fll->d_filter_size+noutput_items);

    float error;
    gr_complex nco_out;
    gr_complex out_upper, out_lower;
    copy(d_qpsk_fll->d_output_hist.begin(),d_qpsk_fll->d_output_hist.end(),d_qpsk_fll->d_fllbuffer.begin());

    for(int i = 0; i < noutput_items; i++) {
      nco_out = exp(std::complex<double>(0,d_qpsk_fll->d_phase));
      d_qpsk_fll->d_fllbuffer[i+d_qpsk_fll->d_filter_size] = received_samples[i] * (std::complex<double>)nco_out;
      // Perform the dot product of the output with the filters
      out_upper = 0;
      out_lower = 0;

      out_upper = d_qpsk_fll->d_filter_lower->filter(&d_qpsk_fll->d_fllbuffer[i]);
      out_lower = d_qpsk_fll->d_filter_upper->filter(&d_qpsk_fll->d_fllbuffer[i]);
    ...
    copy(d_qpsk_fll->d_fllbuffer.begin(), d_qpsk_fll->d_fllbuffer.begin()+noutput_items,tmp2);
    copy(d_qpsk_fll->d_fllbuffer.begin()+noutput_items,d_qpsk_fll->d_fllbuffer.begin()+noutput_items+d_qpsk_fll->d_filter_size,d_qpsk_fll->d_output_hist.begin());
    ...
    while((i < noutput_items2) && (count < nrequired)) {
      ...
      d_clock_qpsk_pfb->d_rate_f = gr_branchless_clip(d_clock_qpsk_pfb->d_rate_f, d_clock_qpsk_pfb->d_max_dev);
...
bool Modem_QPSK::preamble_detection(bvec received_bits,bvec &received_bits2,int &preamble_start){
    ...
    while((i<int(received_bits.size())-96)&&(preamble_ok==false)){
        ...
        if((preamble_bits==received_bits_rotated1.get(i,i+24-1))&&(preamble_bits==received_bits_rotated1.get(i+72,i+96-1))){
            preamble_ok=true;
            received_bits2.set_size(received_bits.size()-i);
            received_bits2=received_bits_rotated1.get(i,received_bits.size()-1);
            preamble_start=i;
        }
        if((preamble_bits==received_bits_rotated2.get(i,i+24-1))&&(preamble_bits==received_bits_rotated2.get(i+72,i+96-1))){
            preamble_ok=true;
            received_bits2.set_size(received_bits.size()-i);
            received_bits2=received_bits_rotated2.get(i,received_bits.size()-1);
            preamble_start=i;
        }
        if((preamble_bits==received_bits_rotated3.get(i,i+24-1))&&(preamble_bits==received_bits_rotated3.get(i+72,i+96-1))){
            preamble_ok=true;
            received_bits2.set_size(received_bits.size()-i);
