...
vec Modem_GMSK::pfb_clock_recovery(vec received_samples){
    ...
    digital_pfb_clock_sync_fff_sptr d_clock_gmsk_pfb; /*! Time Recovery */
    ...
    d_clock_gmsk_pfb = digital_make_pfb_clock_sync_fff(OF,1.0,taps);
    ...
    while((i < noutput_items2) && (count < nrequired)) {
      ...
      d_clock_gmsk_pfb->d_rate_f = gr_branchless_clip(d_clock_gmsk_pfb->d_rate_f, d_clock_gmsk_pfb->d_max_dev);
