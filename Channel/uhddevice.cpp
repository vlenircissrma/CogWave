...
void UHDDevice::waiting_time(double time_value){
...
cvec UHDDevice::readsamples(int Nsamples,double timestamp)
{
    ...
    int num_rx_samps=0;
     ...
     usrp->issue_stream_cmd(stream_cmd);
     num_rx_samps=rx_stream->recv(&rx_buff(0), rx_buff.size(), rx_md, timestamp+rx_buff.size()/tx_rate);
...
void UHDDevice::sendsamplesnow(cvec tx_buff){

    uhd::tx_metadata_t tx_md;
    tx_md.start_of_burst = true;
    tx_md.end_of_burst = true;
    tx_md.has_time_spec = false;
    tx_stream->send(&tx_buff(0), tx_buff.size(), tx_md, tx_buff.size()/tx_rate);
