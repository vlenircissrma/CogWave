...
Waveform_FDD_RX::Waveform_FDD_RX(Ui_MainWindow *ui)
{
...
void Waveform_FDD_RX::update_uhd(){
               ...
               if(packet->is_ber_count==true)
                ...
                if(waveform==2){
                    if(blindofdm->time_offset_estimate>Nfft-1)
                        correction=Number_of_received_symbols+(blindofdm->time_offset_estimate-(Nfft+Ncp))+int(preamble_start/sum_mask)*(Nfft+Ncp);
                    else
                        correction=Number_of_received_symbols+(blindofdm->time_offset_estimate)+int(preamble_start/sum_mask)*(Nfft+Ncp);
                }
                if(waveform==3)
