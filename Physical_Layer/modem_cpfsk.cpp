...
vec Modem_CPFSK::pfb_clock_recovery(vec received_samples){
    ...
    gr::digital::pfb_clock_sync_fff_impl d_clock_cpfsk_pfb(OF,1.0,taps);
    ...
    while((i < noutput_items2) && (count < nrequired)) {
      ...
      d_clock_cpfsk_pfb.d_rate_f = gr::branchless_clip(d_clock_cpfsk_pfb.d_rate_f, d_clock_cpfsk_pfb.d_max_dev);
