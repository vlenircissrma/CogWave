...
QPSK_BER_Test::QPSK_BER_Test()
{
    ...
    cvec received_chips;
    double norm_fading;
    BERC berc,berc1,berc2,berc3,berc4;
...
/*          fading.set_row(1,norm_fading*channel2);
            ...
            bvec received_bits_rotated1=qpsk.demodulate_bits(qpsk.modulate_bits(received_bits)*exp(std::complex<double>(0,pi/2)));
            bvec received_bits_rotated2=qpsk.demodulate_bits(qpsk.modulate_bits(received_bits)*exp(std::complex<double>(0,pi)));
            bvec received_bits_rotated3=qpsk.demodulate_bits(qpsk.modulate_bits(received_bits)*exp(std::complex<double>(0,3*pi/2)));

            //First Receiver M&M+Costas
            /*berc1.count(transmitted_bits.get(2,NumOfBits-15), received_bits.get(16,NumOfBits-1));
            ber(i) = berc1.get_errorrate();
            berc=berc1;
            berc2.count(transmitted_bits.get(2,NumOfBits-15), received_bits_rotated1.get(16,NumOfBits-1));
            if(berc2.get_errorrate()<ber(i)){
                ber(i) = berc2.get_errorrate();
                berc=berc2;
            }
            berc3.count(transmitted_bits.get(2,NumOfBits-15), received_bits_rotated2.get(16,NumOfBits-1));
            if(berc3.get_errorrate()<ber(i)){
                ber(i) = berc3.get_errorrate();
                berc=berc3;
            }
            berc4.count(transmitted_bits.get(2,NumOfBits-15), received_bits_rotated3.get(16,NumOfBits-1));
            if(berc4.get_errorrate()<ber(i)){
                ber(i) = berc4.get_errorrate();
                berc=berc4;
            }*/

            //Second receiver PFB+Costas
            berc1.count(transmitted_bits.get(2,NumOfBits-9), received_bits.get(8,NumOfBits-1));
            ber(i) = berc1.get_errorrate();
            berc=berc1;
            berc2.count(transmitted_bits.get(2,NumOfBits-9), received_bits_rotated1.get(8,NumOfBits-1));
            if(berc2.get_errorrate()<ber(i)){
                ber(i) = berc2.get_errorrate();
                berc=berc2;
            }
            berc3.count(transmitted_bits.get(2,NumOfBits-9), received_bits_rotated2.get(8,NumOfBits-1));
            if(berc3.get_errorrate()<ber(i)){
                ber(i) = berc3.get_errorrate();
                berc=berc3;
            }
            berc4.count(transmitted_bits.get(2,NumOfBits-9), received_bits_rotated3.get(8,NumOfBits-1));
            if(berc4.get_errorrate()<ber(i)){
                ber(i) = berc4.get_errorrate();
                berc=berc4;
            }
