...
cvec Modem_QPSK::modulate(bvec data_packet)
{
    ...
    QPSK qpsk;
    cvec mapped_transmitted_symbols=qpsk.modulate_bits(data_packet);
...
bvec Modem_QPSK::demodulate(cvec rx_buff, cvec &out)
{
    ...
    gr::digital::mpsk_receiver_cc::sptr qpsk_receiver;
    qpsk_receiver=gr::digital::mpsk_receiver_cc::make(M,theta,loop_bw,fmin,fmax,mu,gain_mu,omega,gain_omega,omega_rel);

    /*//SECOND RECEIVER: FREQUENCY LOCKED LOOP + PFB CLOCK SYNC + COSTAS LOOP

    //FREQUENCY LOCKED LOOP
    float loop_bw=2*pi/100;
    gr::digital::fll_band_edge_cc::sptr qpsk_fll;
    qpsk_fll=gr::digital::fll_band_edge_cc::make(OF,roll_off,55,loop_bw);
    ...
    for(int i=0;i<rrc_response_size;i++)
        taps[i]=rrc_response[i];
    gr::digital::pfb_clock_sync_ccf::sptr qpsk_pfb;
    qpsk_pfb=gr::digital::pfb_clock_sync_ccf::make(OF,loop_bw,taps);

    //COSTAS LOOP
    gr::digital::costas_loop_cc::sptr qpsk_costas;
    ...
    tb->connect(injector_demodulator,0,qpsk_receiver,0);
    tb->connect(qpsk_receiver,0,sniffer_demodulator,0);

    /*//CONNECT SECOND RECEIVER
    tb->connect(injector_demodulator,0,qpsk_fll,0);
    tb->connect(qpsk_fll,0,qpsk_pfb,0);
    tb->connect(qpsk_pfb,0,qpsk_costas,0);
    tb->connect(qpsk_costas,0,sniffer_demodulator,0);*/
